...
abstract class Serialize[A] { self =>
  ...
  final def xmap[B](f: A => B, g: B => A): Serialize[B] =
    new Serialize[B] {
...
object Serialize {
  ...
  implicit val bool: Serialize[Boolean] = new Serialize[Boolean] {
    ...
    def unpack(v: MessagePack): Attempt[Boolean] =
      v match {
  ...
  implicit val int: Serialize[Int] = new Serialize[Int] {
    ...
    def unpack(v: MessagePack): Attempt[Int] =
      v match {
  ...
  implicit val long: Serialize[Long] = new Serialize[Long] {
    ...
    def unpack(v: MessagePack): Attempt[Long] =
      v match {
        case MPositiveFixInt(n) => Attempt.successful(n)
        case MNegativeFixInt(n) => Attempt.successful(n)
        case MUInt8(n) => Attempt.successful(n)
  ...
  implicit val float: Serialize[Float] = new Serialize[Float] {
    ...
    def unpack(v: MessagePack): Attempt[Float] =
      v match {
  ...
  implicit val double: Serialize[Double] = new Serialize[Double] {
    ...
    def unpack(v: MessagePack): Attempt[Double] =
      v match {
  ...
  implicit val string: Serialize[String] = new Serialize[String] {
    def pack(v: String): Attempt[MessagePack] =
      Attempt.successful {
    ...
    def unpack(v: MessagePack): Attempt[String] =
      v match {
  ...
  implicit val binary: Serialize[ByteVector] = new Serialize[ByteVector] {
    def pack(v: ByteVector): Attempt[MessagePack] =
      Attempt.successful {
        val len = v.size
    ...
    def unpack(v: MessagePack): Attempt[ByteVector] =
      v match {
  ...
  implicit def array[A](implicit S: Serialize[A]): Serialize[Vector[A]] =
    new Serialize[Vector[A]] {
      ...
      def unpack(v: MessagePack): Attempt[Vector[A]] =
        v match {
  ...
  implicit def map[A, B](implicit S: Serialize[A], T: Serialize[B]): Serialize[Map[A, B]] =
    new Serialize[Map[A, B]] {
      ...
      def unpack(v: MessagePack): Attempt[Map[A, B]] =
        v match {
  ...
  def extension[A](code: ByteVector)(implicit S: Codec[A]): Serialize[A] =
    new Serialize[A] {
      def pack(v: A): Attempt[MessagePack] =
